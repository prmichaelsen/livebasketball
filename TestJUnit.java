...
public class TestJUnit {
	...
	public void beforeEach(){
		...
		while(true){
			...
			//homeScores.add(new ArrayList<Integer>(Arrays.asList(new Integer[]{10,10,10,0})));	
			//homeTeams.add("Home Team" );
			//awayScores.add(new ArrayList<Integer>(Arrays.asList(new Integer[]{9,9,9,0})));	
			//awayTeams.add("Away Team");

			//homeScores.add(new ArrayList<Integer>(Arrays.asList(new Integer[]{9,10,10,0})));	
			//homeTeams.add("No Home Team" );
			//awayScores.add(new ArrayList<Integer>(Arrays.asList(new Integer[]{9,9,9,0})));	
			//awayTeams.add("No Away Team");

			//homeScores.add(new ArrayList<Integer>(Arrays.asList(new Integer[]{10,10,10,2})));	
			//homeTeams.add("No 2 Home Team" );
			//awayScores.add(new ArrayList<Integer>(Arrays.asList(new Integer[]{9,9,9,0})));	
			//awayTeams.add("No 2 Away Team");

			//homeScores.add(new ArrayList<Integer>(Arrays.asList(new Integer[]{10,10,2})));	
			//homeTeams.add("No 3 Home Team" );
			//awayScores.add(new ArrayList<Integer>(Arrays.asList(new Integer[]{9,9,0})));	
			//awayTeams.add("No 3 Away Team");

			//homeScores.add(new ArrayList<Integer>(Arrays.asList(new Integer[]{10,10,10,0})));	
			//homeTeams.add("2 Home Team" );
			//awayScores.add(new ArrayList<Integer>(Arrays.asList(new Integer[]{9,9,5,0})));	
			//awayTeams.add("2 Away Team");

			/*if(false ){//homeScores.size() == awayScores.size()){
				for(int i = 0; i < homeScores.size() ; i++ ){
					List<Integer> home = homeScores.get(i);
					List<Integer> away = awayScores.get(i);
					List<Integer> differences = new ArrayList<Integer>();
					boolean hasPositive = false;
					boolean hasNegative = false;
					boolean hasZero = false; 
					//boolean roundStarting = false;
					//if( home.get(home.size()-1) == 0 && away.get(away.size()-1) == 0){
						//	roundStarting = true;
						for(int j = 0; j < home.size() - 1; j++){
							int diff = home.get(j) - away.get(j); 
							if(diff > 0){
								hasPositive = true;		
							}
							else if(diff < 0){
						...
						String match_name = homeTeams.get(i) + " vs. " + awayTeams.get(i);
						if( (!hasPositive || !hasNegative)  && !hasZero ){ 
							if( !notifications.containsKey(match_name) ){
								notifications.put(match_name, true);
								System.out.println("match: " + match_name ); 
								if(playSounds){
								Thread beepThread = new Thread(new Runnable(){
									public void run(){
										try{
							...
							notifications.remove(match_name); 
						}
					//}
				} 
			} */
